...
每次写小红书前,先输出一个结构化构思卡片:

**1 一句话定位**:这篇笔记给读者的核心价值是什么?

**2 标题备选**(至少 3 个):
- 每个标题标注类型(争议/立场/反问/数字/悬念)
- 推荐哪个 + 理由

**3 内容结构**:
- 开头钩子(1-2 句,写出具体内容)
- 正文框架(列出 2-3 个要点,每个一句话)
- 互动提问(写出具体问题)

**4 配图规划**(多图笔记必填):
- 几张图,每张图什么内容,什么风格
- 参考 `references/presets.md` 的预设推荐

**5 风险标注**:是否剧透 / 引战边界 / 版权风险 / 平台敏感词

**确认流程**:输出构思卡片 → 老板确认标题和方向 → 确认后再写正文。先对齐方向再动笔,避免写完推倒重来。
...
### 🔴 §4.2.5 真实性校验（生图前，事实类选题必过）

> 来源：ATA《小红书 AIGC 图文卡片 skill 生产实录》。风格不统一只是"有点 AI 味"，事实错误是直接砸信任。尤其旅游/消费/知识分享类，混进一两条错信息整组内容立刻廉价。

**适用场景**：含地点/时间/数量/价格/距离/产品信息的选题（旅游攻略、好物测评、数据干货、科普）。纯情绪/观点/个人故事可跳过。

**第一层：生图前先查硬事实**（用 web_search）：
- 地点、时间、数量、产品信息、价格区间、距离和路线、明显不合理的因果关系
- 基础判断：来源靠不靠谱、有没有夸大表达、有没有把主观感受写成绝对结论
- 🔴 **不要在信息不确定时让模型继续猜**：能查就查，查不清就标风险，别硬编
- **web_search 重试纪律**：最多重试 3 次；拿不到确定信息就明确标成"待用户确认"，不把不确定伪装成确定事实

**第二层：生图后复核**（交付前，见 §Final）：
- 图内文字有没有被模型写坏/错字/漏字（图像模型渲染文字常自己加戏）
- 正文事实是不是还成立、标签和正文是不是对得上、图和文是不是在说同一件事
- 用 `image` 工具（视觉模型）或人工逐张核图内文字
...
### 🔴 §4.3.1 数字契约门禁(N+2,生图前必过)

> 来源:ATA《小红书 AIGC 图文卡片 skill 生产实录》。封面标题里的数字是**系统契约**,不是文案修辞。封面写「6 大草原」正文只有 4 个 = 用户立刻判定 AI 乱写。

图卡系列规划完成后、**调用生图前**,必须过数字契约门禁:

- **从封面标题提取数字 N**(如「6 大草原」→ N=6,「5 步搞定」→ N=5)
- **内容页数量必须 = N**
- **总图数必须 = N + 2**(封面 + N 张内容 + 互动/结尾页)
- **对不上直接阻塞,不准进入生图阶段**,回退重新规划到一致

规划阶段(还没生图)用 `--content-pages` 核内容页:
```bash
python3 <WORKSPACE>/scripts/check-number-contract.py <文案.md> --content-pages <规划内容页数>
```
生图后用 `--image-dir` 核总图(已并入 §Final 机械验证):
```bash
python3 <WORKSPACE>/scripts/check-number-contract.py <文案.md> --image-dir <配图目录>
```
- exit 0 = 契约成立或标题无数字承诺(自动放行);exit 1 = 违约阻塞
- 这套思路可外扩:「三大特性」必须真有 3 个点、「5 步教程」必须真有 5 步、「A vs B」必须两边对称展开
- **原则**:模型只要对外说了什么,系统就逼它兑现。
...
### §5.5: Obsidian 归档（交付后自动执行）

将文案和配图归档到 Obsidian `创作/小红书/`，保留完整创作记录。

```bash
bash scripts/archive-to-obsidian.sh --platform xhs \
  --article temp/xhs/<text>.md \
  --images temp/xhs/images/*.jpg
```

- 自动注入 `archived_at` 和 `platform` 到 frontmatter
- 图卡复制到 `创作/小红书/images/`，文内引用自动替换
- 失败不阻塞主流程，记录警告即可
...
**🔴 生图后文字复核（§4.2.5 第二层落地，事实类选题不可跳）**：
skill-verify 通过后，交付前用 `image` 工具逐张核：
- 图内文字是否被模型写坏/错字/漏字（尤其小字号详情）
- 图文是否一致（标题数字 vs 实际内容、标签 vs 正文）
- 发现写坏→ 重生该张；发现事实错误→ 回到 §4.2.5 重查后重生
